...
//musics
const bgMusic = new Audio('./music/bg.wav')
bgMusic.loop = true
const winMusic = new Audio('./music/win.wav')
const loseMusic = new Audio('./music/lose.wav')
...
class Player {
    constructor(name = "Jugador", radius = .25, posY = 0, color = 0xffffff){
    ...
    check(){
        ...
        if(!dallFacingBack && this.playerInfo.velocity > 0){
            ...
            if(DEAD_PLAYERS == players.length){
                text.innerText = "Todos perdieron!!!"
                gameStat = "ended"
            }
        ...
        if(this.playerInfo.positionX < end_position + .7){
            text.innerText = this.playerInfo.name + " Se salvo!!!"
            this.playerInfo.isDead = true
            this.stop()
            SAFE_PLAYERS++
            winMusic.play()
            if(SAFE_PLAYERS == players.length){
                text.innerText = "Todos estan a salvo!!"
                gameStat = "ended"
            }
            if(DEAD_PLAYERS + SAFE_PLAYERS == players.length){
                gameStat = "ended"
...
const player1 = new Player("Jugador 1", .25, .3, 0xD1FFC6)
const player2 = new Player("Jugador 2", .25, -.3, 0xFFCFD2)
...
function start(){
    ...
    setTimeout(() => {
        if(gameStat != "ended"){
            text.innerText = "Se acabo el tiempo!!!"
            loseMusic.play()
            gameStat = "ended"
        }
    }, TIME_LIMIT * 1000)
...
text.innerText = "Creado por Lucio Sianca" 
